...
def search():
    ...
    if user:
        return jsonify({
            ...
            'friends': "Friends" if (current_user.is_following(user) and user.is_following(current_user)) else "Not Friends",
            'is_following': current_user.is_following(user),  # Current user follows this user
            'is_follower': user.is_following(current_user),   # This user follows current user
            'are_friends': current_user.is_following(user) and user.is_following(current_user)  # Mutual following
        })
    else:
